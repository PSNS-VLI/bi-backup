...
defineOptions({
  name: ChartName.COLUMN_PERCENTAGE
})
...
let labelTotal = { text: (data) => {}, textAlign: 'start' }
...
const dataTagConfigChange = (store) => {
  ...
  if (dataTagContent?.includes('measurement') && dataTagContent?.includes('proportion')) {
    labelValue.text = (data) => `${data[valueY.value]}（${data['__BI_PERCENT_STR_VALUE_KEY__']}）`
...
const seriesConfigChange = (store) => {
  ...
  if (seriesConfigShowDataTag) {
    ...
    labelValue.text = (data) =>
      data.__BI_COLOR_KEY__ === Object.keys(seriesConfigLabel)[0] ? data[valueY.value] : ''
  }
  // 切换标签 清空之前的 ？
}
</script>
